...
| --- | --- | --- |
| Rights-clear conditional-reasoning content | 12 original CC BY 4.0 draft records, checksum-bound manifest, public/private projection, and fail-closed draft validator in `domains/content/judgment/lumi-conditional-reasoning-v0`. The third review draft remediates the AI pre-review's `changes_requested`: P02 now makes the necessary→sufficient error explicit before mapping it to M-ROLE support, and T02 removes a time-sequence analogy. Current reviewer materials are `JUDGMENT_PACK_MANUAL_REVIEW_R3.md` plus the matching spreadsheet. | Two real, distinct logic and editorial/rights reviewer attestations must bind the exact **third-draft** manifest, artifact, and record hashes. The repository draft is deliberately not launchable. |
