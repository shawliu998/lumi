...
class XingceAuthoredDraftTests(unittest.TestCase):
    def test_checked_in_drafts_validate_but_cannot_be_registered(self) -> None:
        roots = (
            ...
            CONTENT_ROOT / "judgment" / "lumi-event-ordering-v0",
            ...
            CONTENT_ROOT / "political_theory" / "lumi-party-history-v0",
            CONTENT_ROOT / "political_theory" / "lumi-theoretical-system-v0",
            CONTENT_ROOT / "political_theory" / "lumi-mao-thought-v0",
            CONTENT_ROOT / "political_theory" / "lumi-political-current-affairs-v0",
        )
        for root in roots:
    ...
    def test_history_humanity_draft_keeps_registration_time_distinct_from_origin_before_transfer(self) -> None:
        pack = load_xingce_adaptive_pack(CONTENT_ROOT / "common_knowledge" / "lumi-history-humanity-v0")
        entry = diagnose_entry(
            pack["records"], scorer=pack["scorer"], entry_record_id="D01", observation=Observation("D", "medium", 17)
        )
        probe = resolve_probe(
            pack["records"], scorer=pack["scorer"], entry=entry, observation=Observation("B", "low", 9)
        )
        self.assertEqual(probe.teaching_record_id, "T-CHRON")
        self.assertTrue(all(row["status"] == "unconfirmed" for row in probe.evidence_updates))
        transfer = independent_transfer_proposal(
            pack["records"], scorer=pack["scorer"], entry=entry, probe=probe, observation=Observation("B", "high", 12)
        )
    ...
    def test_law_draft_treats_scope_and_subject_as_unconfirmed_candidates_until_transfer(self) -> None:
        pack = load_xingce_adaptive_pack(CONTENT_ROOT / "common_knowledge" / "lumi-law-v0")
        entry = diagnose_entry(
            pack["records"], scorer=pack["scorer"], entry_record_id="D01", observation=Observation("B", "medium", 20)
        )
        probe = resolve_probe(
            pack["records"], scorer=pack["scorer"], entry=entry, observation=Observation("C", "low", 10)
        )
        self.assertEqual(probe.teaching_record_id, "T-SCOPE")
        self.assertTrue(all(row["status"] == "unconfirmed" for row in probe.evidence_updates))
        transfer = independent_transfer_proposal(
            pack["records"], scorer=pack["scorer"], entry=entry, probe=probe, observation=Observation("B", "high", 15)
        )
        self.assertTrue(transfer["eligible"])

    def test_economics_draft_separates_direction_from_macro_overreach_before_transfer(self) -> None:
        pack = load_xingce_adaptive_pack(CONTENT_ROOT / "common_knowledge" / "lumi-economics-v0")
        entry = diagnose_entry(
            pack["records"], scorer=pack["scorer"], entry_record_id="D01", observation=Observation("B", "medium", 19)
        )
        probe = resolve_probe(
            pack["records"], scorer=pack["scorer"], entry=entry, observation=Observation("D", "low", 9)
        )
        self.assertEqual(probe.teaching_record_id, "T-INDICATOR")
        self.assertTrue(all(row["status"] == "unconfirmed" for row in probe.evidence_updates))
        transfer = independent_transfer_proposal(
            pack["records"], scorer=pack["scorer"], entry=entry, probe=probe, observation=Observation("A", "high", 14)
        )
