...
from tools.verify_xingce_review_workbook import ATTESTATION_SCHEMA
...
def create(source: Path, output: Path, workbook: Path, attestations: Path) -> dict[str, Any]:
    ...
    if signed.get("schema_version") != ATTESTATION_SCHEMA:
        raise ValueError("attestations must be generated by verify_xingce_review_workbook.py")
    source_pack = signed.get("source_pack")
    workbook_binding = signed.get("review_workbook")
    manifest_source = read(source / "manifest.json")
    if not isinstance(source_pack, dict) or source_pack != {
        "pack_id": manifest_source.get("pack_id"),
        "pack_version": manifest_source.get("pack_version"),
        "records_sha256": sha(source / "records.json"),
    }:
        raise ValueError("attestations do not bind this exact source pack")
    if not isinstance(workbook_binding, dict) or workbook_binding != {"name": workbook.name, "sha256": sha(workbook)}:
        raise ValueError("attestations do not bind this exact review workbook")
